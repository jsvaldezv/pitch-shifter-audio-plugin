...
namespace stftpitchshift
{
    ...
    public:
        ...
        STFT (const size_t framesize, const size_t hopsize, const bool chronometry = false) : STFT (std::make_shared<RFFT>(), std::make_tuple (framesize, framesize), hopsize, chronometry)
        {
        }

        STFT (const std::shared_ptr<FFT> fft, const size_t framesize, const size_t hopsize, const bool chronometry = false) : STFT (fft, std::make_tuple (framesize, framesize), hopsize, chronometry)
        {
        }

        STFT (const std::tuple<size_t, size_t> framesize, const size_t hopsize, const bool chronometry = false) : STFT (std::make_shared<RFFT>(), framesize, hopsize, chronometry)
        {
        }

        STFT (const std::shared_ptr<FFT> fft, const std::tuple<size_t, size_t> framesize, const size_t hopsize, const bool chronometry = false) : fft (fft),
                                                                                                                                                  framesize (framesize),
                                                                                                                                                  hopsize (hopsize),
                                                                                                                                                  chronometry (chronometry)
        {
            ...
            if (analysis_window_size < synthesis_window_size)
            {
                throw std::runtime_error (
                    "Analysis window (" + std::to_string (analysis_window_size) + ") must be greater" + " or equal to synthesis window (" + std::to_string (synthesis_window_size) + ")!");
            }
            ...
            const T unitygain = hopsize / std::inner_product (window.analysis.begin(), window.analysis.end(), window.synthesis.begin(), T (0));

            std::transform (window.synthesis.begin(), window.synthesis.end(), window.synthesis.begin(), [unitygain] (T value)
            {
                return value * unitygain;
            });
