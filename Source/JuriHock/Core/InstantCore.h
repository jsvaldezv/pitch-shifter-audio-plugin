...
private:
    ...
    static Y transform (const X x)
    {
        return static_cast<Y> (x);
    }

    void process (const std::span<const float> input, const std::span<float> output, std::function<void (std::span<double> x, std::span<double> y)> callback);
};
